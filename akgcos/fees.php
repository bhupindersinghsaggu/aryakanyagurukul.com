<?php

include('_web/header.php')
?>


<section class="page-title">
    <div class="container">
        <div class="row">
            <div class="col-lg-8">
                <div class="theme-breadcrumb-box">
                    <h1>Fees Details</h1>

                </div>
            </div>
        </div>
        <!-- / .row -->
    </div>
    <!-- / .container -->
</section>

<section>
    <div class="container">
        <div class="row">
            <div class="col-lg-12 col-12 ps-lg-10 order-lg-1">
                <div class="table-heading" style="text-align:left" ;>
                    <h2>Fee structure (2024-2025)</h2><br>
                    <h5> Arya kanya Gurukul Sanskrit Mahavidyalaya Mor Majra (Karnal)</h5>
                    <br>
                    <table class="table table-striped">
                        <tbody>
                            <tr>
                                <td width="101">
                                    <p><strong>S. No.</strong></p>
                                </td>
                                <td width="128">
                                    <p><strong>Class</strong></p>
                                </td>
                                <td width="164">
                                    <p><strong>Composite Fee</strong></p>
                                </td>
                                <td width="133">
                                    <p><strong>Hostel Fees</strong></p>
                                </td>
                                <td width="128">
                                    <p><strong>Total Fees</strong></p>
                                </td>
                            </tr>
                            <tr>
                                <td width="101">
                                    <p><strong>1</strong></p>
                                </td>
                                <td width="128">
                                    <p>Shastri-I</p>
                                </td>
                                <td width="164">
                                    <p>21230</p>
                                </td>
                                <td width="133">
                                    <p>36000</p>
                                </td>
                                <td width="128">
                                    <p>57230</p>
                                </td>
                            </tr>
                            <tr>
                                <td width="101">
                                    <p><strong>2</strong></p>
                                </td>
                                <td width="128">
                                    <p>Shastri-II</p>
                                </td>
                                <td width="164">
                                    <p>21230</p>
                                </td>
                                <td width="133">
                                    <p>36000</p>
                                </td>
                                <td width="128">
                                    <p>57230</p>
                                </td>
                            </tr>
                            <tr>
                                <td width="101">
                                    <p><strong>3</strong></p>
                                </td>
                                <td width="128">
                                    <p>Shastri-III</p>
                                </td>
                                <td width="164">
                                    <p>21230</p>
                                </td>
                                <td width="133">
                                    <p>36000</p>
                                </td>
                                <td width="128">
                                    <p>57230</p>
                                </td>
                            </tr>
                            <tr>
                                <td width="101">
                                    <p><strong>4</strong></p>
                                </td>
                                <td width="128">
                                    <p>Visharda-I</p>
                                </td>
                                <td width="164">
                                    <p>21230</p>
                                </td>
                                <td width="133">
                                    <p>30,000</p>
                                </td>
                                <td width="128">
                                    <p>51230</p>
                                </td>
                            </tr>
                            <tr>
                                <td width="101">
                                    <p><strong>5</strong></p>
                                </td>
                                <td width="128">
                                    <p>Visharda-II</p>
                                </td>
                                <td width="164">
                                    <p>21230</p>
                                </td>
                                <td width="133">
                                    <p>30,000</p>
                                </td>
                                <td width="128">
                                    <p>51230</p>
                                </td>
                            </tr>
                        </tbody>
                    </table>
                    <p><strong>Installment Wise Fees Structure:</strong></p>
                    <table class="table table-striped">
                        <tbody>
                            <tr>
                                <td width="92">
                                    <p><strong>S. No.</strong></p>
                                </td>
                                <td width="121">
                                    <p><strong>Class</strong></p>
                                </td>
                                <td width="116">
                                    <p><strong>Total Fees</strong></p>
                                </td>
                                <td width="108">
                                    <p><strong>I<sup>st </sup>Installment</strong></p>
                                </td>
                                <td width="108">
                                    <p><strong>II<sup>nd </sup>Installment</strong></p>
                                </td>
                                <td width="108">
                                    <p><strong>III<sup>rd </sup>Installment</strong></p>
                                </td>
                            </tr>
                            <tr>
                                <td width="92">
                                    <p><strong>1</strong></p>
                                </td>
                                <td width="121">
                                    <p>Shastri-I</p>
                                </td>
                                <td width="116">
                                    <p>57230</p>
                                </td>
                                <td width="108">
                                    <p>20,000</p>
                                </td>
                                <td width="108">
                                    <p>20,000</p>
                                </td>
                                <td width="108">
                                    <p>17,230</p>
                                </td>
                            </tr>
                            <tr>
                                <td width="92">
                                    <p><strong>2</strong></p>
                                </td>
                                <td width="121">
                                    <p>Shastri-II</p>
                                </td>
                                <td width="116">
                                    <p>57230</p>
                                </td>
                                <td width="108">
                                    <p>20,000</p>
                                </td>
                                <td width="108">
                                    <p>20,000</p>
                                </td>
                                <td width="108">
                                    <p>17,230</p>
                                </td>
                            </tr>
                            <tr>
                                <td width="92">
                                    <p><strong>3</strong></p>
                                </td>
                                <td width="121">
                                    <p>Shastri-III</p>
                                </td>
                                <td width="116">
                                    <p>57230</p>
                                </td>
                                <td width="108">
                                    <p>20,000</p>
                                </td>
                                <td width="108">
                                    <p>20,000</p>
                                </td>
                                <td width="108">
                                    <p>17,230</p>
                                </td>
                            </tr>
                            <tr>
                                <td width="92">
                                    <p><strong>4</strong></p>
                                </td>
                                <td width="121">
                                    <p>Visharda-I</p>
                                </td>
                                <td width="116">
                                    <p>51230</p>
                                </td>
                                <td width="108">
                                    <p>20,000</p>
                                </td>
                                <td width="108">
                                    <p>20,000</p>
                                </td>
                                <td width="108">
                                    <p>11,230</p>
                                </td>
                            </tr>
                            <tr>
                                <td width="92">
                                    <p><strong>5</strong></p>
                                </td>
                                <td width="121">
                                    <p>Visharda-II</p>
                                </td>
                                <td width="116">
                                    <p>51230</p>
                                </td>
                                <td width="108">
                                    <p>20,000</p>
                                </td>
                                <td width="108">
                                    <p>20,000</p>
                                </td>
                                <td width="108">
                                    <p>11,230</p>
                                </td>
                            </tr>
                        </tbody>
                    </table>
                    <p><strong>Payment Flow:</strong></p>
                    <table width="646">
                        <tbody>
                            <tr>
                                <td width="288">
                                    <p><strong>Installment</strong></p>
                                </td>
                                <td width="358">
                                    <p><strong>Details</strong></p>
                                </td>
                            </tr>
                            <tr>
                                <td width="288">
                                    <p><strong>First Installment</strong></p>
                                </td>
                                <td width="358">
                                    <p>at the time of admission</p>
                                </td>
                            </tr>
                            <tr>
                                <td width="288">
                                    <p><strong>Second Installment</strong></p>
                                </td>
                                <td width="358">
                                    <p>from 1st to 20th November</p>
                                </td>
                            </tr>
                            <tr>
                                <td width="288">
                                    <p><strong>Third Installment</strong></p>
                                </td>
                                <td width="358">
                                    <p>from 1st to 20th March</p>
                                </td>
                            </tr>
                        </tbody>
                    </table>
                    <p><strong>Payment Guidelines:</strong>
                        <br>1. A Punjab National Bank branch is available at the Gurukul
                        campus to help with account opening and fee payments.
                        <br>2. All installments must be paid during the
                        working days of the college.
                    </p>
                </div>
            </div>
        </div>
    </div>
</section>
<?php

include('_web/footer.php')
?>


<?php

include('_web/footer.php')
?>