<?php

include('_web/header.php');

?>

<section class="breadcrumb-area d-flex p-relative align-items-center" style="background-image: url(img/bg/bdrc-bg.png)">
    <div class="container">
        <div class="row align-items-center">
            <div class="col-xl-12 col-lg-12">
                <div class="breadcrumb-wrap text-left">
                    <div class="breadcrumb-title">
                        <h2> Faculty</h2>
                    </div>
                </div>
            </div>
            <!-- <div class="breadcrumb-wrap2">
            <nav aria-label="breadcrumb">
              <ol class="breadcrumb">
                <li class="breadcrumb-item"><a href="index.html">Home</a></li>
                <li class="breadcrumb-item active" aria-current="page">
                  Courses
                </li>
              </ol>
            </nav>
          </div> -->
        </div>
    </div>
</section>
<section style="padding-top: 50px;">
    <div class="container">
        <div class="row">
            <div class="teachingfaculty" id="teachingfaculty">

                <h3>
                    Non-Teaching Staff
                </h3>
                <table class="table table-striped">

                    <tbody>
                        <tr>
                            <td width="28"><strong>Sr. </strong><br>
                                <strong> No.</strong>
                            </td>
                            <td width="107"><strong>Name &amp; Full</strong><br>
                                <strong> Address</strong>
                            </td>
                            <td width="111"><strong>Designation</strong></td>
                            <td width="175"><strong>Qualification</strong></td>

                            <td width="71"><strong>Date of </strong><br>
                                <strong> Appointment</strong>
                            </td>

                            <td width="81"><strong>Experience </strong></td>
                            <td width="127"><strong>Photograph</strong></td>
                        </tr>
                        <tr>
                            <td width="28"><strong>1</strong></td>
                            <td width="107"><strong>Ms. Seema</strong></td>
                            <td width="111"><strong>Librarian</strong></td>
                            <td width="175"><strong>M.A. M.Lib</strong></td>
                            <td width="152"><strong>18.04.2012
                                </strong>
                            </td>
                            <td width="79"><strong>11 Year
                                </strong></td>
                            <td width="127"><strong><img loading="lazy" class="size-medium wp-image-727 alignleft" src="img/staff/b.ed/seema1.jpg" alt="SEEMA LIBRARY 001" width="243" height="300" srcset="img/staff/b.ed/seema1.jpg" sizes="(max-width: 243px) 100vw, 243px"></strong></td>
                        </tr>
                        <tr>
                            <td width="28"><b>2</b></td>
                            <td width="107"><strong>Mr. Sahil Mehta</strong></td>
                            <td width="111"><strong>Head Clerk</strong></td>
                            <td width="175"><strong>B.Com (Vocational)</strong><br>
                                <strong> M.B.A.</strong>
                            </td>
                            <td width="152"><strong>01.04.2009
                                </strong>
                            </td>
                            <td width="91"><strong>14 Year
                                </strong></td>
                            <td width="127"><img loading="lazy" class="alignnone size-medium wp-image-732" src="img/staff/b.ed/sahil.jpg" alt="SEEMA LIBRARY 001" width="243" height="300" srcset="img/staff/b.ed/sahil.jpg" sizes="(max-width: 243px) 100vw, 243px"></strong></td>
                        </tr>
                        <tr>
                            <td width="28"><b>3</b></td>
                            <td width="107"><strong>Ms. Renu Bala</strong></td>
                            <td width="111"><strong>Office Cum Accounts Assistant</strong></td>
                            <td width="175"><strong>B.A. M.Sc. DCA
                                </strong></td>
                            <td width="152"><strong>01.07.2023
                            </td>
                            <td width="71"><strong>10 Year
                                </strong></td>
                            <td width="127"><img loading="lazy" class="alignnone size-medium wp-image-732" src="img/staff/b.ed/renu.jpg" alt="SEEMA LIBRARY 001" width="243" height="300" srcset="img/staff/b.ed/renu.jpg" sizes="(max-width: 243px) 100vw, 243px"></strong></td>
                        </tr>
                        <tr>
                            <td width="28"><b>4</b></td>
                            <td width="107"><strong>Mr. Manuj</strong></td>
                            <td width="111"><strong>Office Assistant cum
                                    Computer Operator</strong><br>

                            </td>
                            <td width="175"><strong>B.Com</strong></td>
                            <td width="152"><strong>01.12.2023
                                </strong>
                            </td>
                            <td width="81"><strong>.....
                                </strong></td>
                            <td width="127"><img loading="lazy" class="alignnone size-medium wp-image-732" src="img/staff/b.ed/manuj.jpg" alt="SEEMA LIBRARY 001" width="243" height="300" srcset="img/staff/b.ed/manuj.jpg" sizes="(max-width: 243px) 100vw, 243px"></strong></td>
                        </tr>


                        <tr>
                            <td width="28"><b>5</b></td>
                            <td width="107"><strong>Ms. Kusum Devi</strong></td>
                            <td width="111"><strong>Storekeeper Cum Helper
                                </strong></td>
                            <td width="175"><strong>B.A
                                </strong></td>
                            <td width="152"><strong>01.07.2023
                                </strong>
                            </td>
                            <td width="71"><strong>7 Year
                                </strong></td>
                            <td width="127"><img loading="lazy" class="alignnone size-medium wp-image-732" src="img/staff/b.ed/kusum.jpg" alt="SEEMA LIBRARY 001" width="243" height="300" srcset="img/staff/b.ed/kusum.jpg" sizes="(max-width: 243px) 100vw, 243px"></strong></td>
                        </tr>
                        <tr>
                            <td width="28"><b>6</b></td>
                            <td width="107"><strong>Mr. Raj kumar</strong></td>
                            <td width="111"><strong>Gardner
                                </strong></td>
                            <td width="175"><strong>--</strong></td>
                            <td width="152"><strong>01.07.2023
                                </strong>
                            </td>
                            <td width="71"><strong>5 Year
                                </strong></td>
                            <td width="127"><img loading="lazy" class="alignnone size-medium wp-image-732" src="img/staff/b.ed/raj.jpg" alt="SEEMA LIBRARY 001" width="243" height="300" srcset="img/staff/b.ed/raj.jpg" sizes="(max-width: 243px) 100vw, 243px"></strong></td>
                        </tr>
                           <tr>
                            <td width="28"><b>6</b></td>
                            <td width="107"><strong>Mr. Rajroop</strong></td>
                            <td width="111"><strong>Gardner
                                </strong></td>
                            <td width="175"><strong>--</strong></td>
                            <td width="152"><strong>01.07.2023

                                </strong>
                            </td>
                            <td width="71"><strong>5 Year
                                </strong></td>
                            <td width="127"><img loading="lazy" class="alignnone size-medium wp-image-732" src="img/staff/b.ed/rajroop.jpg" alt="SEEMA LIBRARY 001" width="243" height="300" srcset="img/staff/b.ed/rajroop.jpg" sizes="(max-width: 243px) 100vw, 243px"></strong></td>
                        </tr>
                        <tr>
                            <td width="28"><b>7</b></td>
                            <td width="107"><strong>Ms. Dimple
                                </strong></td>
                            <td width="111"><strong>Sweeper
                                </strong></td>
                            <td width="175"><strong>--</strong></td>
                            <td width="152"><strong>01.07.2023
                                </strong>
                            </td>
                            <td width="71"><strong>--
                                </strong></td>
                            <td width="127"><img loading="lazy" class="alignnone size-medium wp-image-732" src="img/staff/b.ed/dimple.jpg" alt="SEEMA LIBRARY 001" width="243" height="300" srcset="img/staff/b.ed/dimple.jpg" sizes="(max-width: 243px) 100vw, 243px"></strong></td>
                        </tr>
                        <tr>
                            <td width="28"><b>8</b></td>
                            <td width="107"><strong>Ms. Suman

                                </strong></td>
                            <td width="111"><strong>Sweeper
                                </strong></td>
                            <td width="175"><strong>--</strong></td>
                            <td width="152"><strong>20.01.2025
                                </strong>
                            </td>
                            <td width="71"><strong>--
                                </strong></td>
                            <td width="127"><img loading="lazy" class="alignnone size-medium wp-image-732" src="img/staff/b.ed/suman.jpg" alt="SEEMA LIBRARY 001" width="243" height="300" srcset="img/staff/b.ed/preeti.jpg" sizes="(max-width: 243px) 100vw, 243px"></strong></td>
                        </tr>
                         <tr>
                            <td width="28"><b>8</b></td>
                            <td width="107"><strong>Ms. Anju

                                </strong></td>
                            <td width="111"><strong>Sweeper
                                </strong></td>
                            <td width="175"><strong>--</strong></td>
                            <td width="152"><strong>01.04.2025
                                </strong>
                            </td>
                            <td width="71"><strong>--
                                </strong></td>
                            <td width="127"><img loading="lazy" class="alignnone size-medium wp-image-732" src="img/staff/b.ed/anju1.jpg" alt="SEEMA LIBRARY 001" width="243" height="300" srcset="img/staff/b.ed/preeti.jpg" sizes="(max-width: 243px) 100vw, 243px"></strong></td>
                        </tr>

                    </tbody>
                </table>
            </div>
        </div>
    </div>
</section>



<?php

include('_web/footer.php');

?>